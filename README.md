...
## ☁️ Salesforce CRM Integration (`POST /api/push-to-salesforce`)

Partner Growth Copilot includes a production-ready Salesforce integration module (`src/integrations/salesforce.ts`) using the **Salesforce OAuth 2.0 Client Credentials Flow**.

### Key Features:
* 🔐 **Automated OAuth 2.0 Auth**: Authenticates against `{SALESFORCE_MY_DOMAIN_URL}/services/oauth2/token` and caches tokens in memory with automatic refresh.
* 🏢 **Query-then-Create Account Pattern**: Runs a SOQL query to check if an Account exists in Salesforce before creating a new one.
* 💼 **Real Opportunity Record Creation**: POSTs mapped opportunity stubs to Salesforce REST API (`/services/data/v60.0/sobjects/Opportunity/`).
* 📅 **Sensible Defaults**: Calculates a 90-day CloseDate default if not provided, maps stage names to standard picklists, and parses numeric Amount fields.

### Environment Variables:
```env
SALESFORCE_CLIENT_ID=your-salesforce-client-id-here
SALESFORCE_CLIENT_SECRET=your-salesforce-client-secret-here
SALESFORCE_MY_DOMAIN_URL=https://yourorg.my.salesforce.com
```

### Curl Example:
```bash
curl -X POST http://localhost:3000/api/push-to-salesforce \
  -H "Content-Type: application/json" \
  -H "x-api-key: pgc-secret-key-123" \
  -d '{
    "opportunity_name": "Acme Retail - IBM watsonx AI Transformation",
